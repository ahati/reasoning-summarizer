#!/bin/bash
# fetch-llama.sh - Downloads and builds llama.cpp for CGo bindings
#
# This script is called by `go generate` to fetch llama.cpp source code
# and build the static libraries needed for the Go bindings.
#
# Usage:
#   ./fetch-llama.sh [version]
#
# Arguments:
#   version - Optional llama.cpp version/commit/tag (default: b5508)
#
...
#
# The script creates:
#   - llama.cpp/ directory with source code
...
if [ -d "$LLAMA_DIR/.git" ]; then
    ...
    if [ -d "$LLAMA_DIR" ]; then
        echo "Removing incomplete llama.cpp directory..."
        rm -rf "$LLAMA_DIR"
    fi
